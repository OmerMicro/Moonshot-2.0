...
function gui_data = create_gui_controls(fig, gui_data)
    ...
    gui_data.voltage_slider = uicontrol('Parent', panel, 'Style', 'slider', ...
                                       'Min', -10000, 'Max', 10000, 'Value', 5000, ...
    ...
    % Max time control
    uicontrol('Parent', panel, 'Style', 'text', ...
              'String', 'Max Simulation Time (s):', ...
              'Position', [20, 100, 180, 20], ...
              'HorizontalAlignment', 'left', 'FontSize', 11);

    gui_data.max_time_edit = uicontrol('Parent', panel, 'Style', 'edit', ...
                                       'String', '5.0', ...
                                       'Position', [200, 100, 60, 20]);

    uicontrol('Parent', panel, 'Style', 'text', ...
              'String', '[0.005 - 0.1]', ...
              'Position', [270, 100, 80, 20], ...
              'HorizontalAlignment', 'left', 'FontSize', 9, ...
              'ForegroundColor', [0.5, 0.5, 0.5]);
    
    % Info text
    uicontrol('Parent', panel, 'Style', 'text', ...
              'String', 'Fixed parameters: Mass=1kg, Tube=0.5m', ...
...
function run_simulation_callback(src, ~)
    ...
    try
        ...
        % Get max_time from edit box
        max_time = str2double(get(gui_data.max_time_edit, 'String'));
        if isnan(max_time) || max_time < 0.005 || max_time > 0.1
            max_time = 0.05; % fallback default
        end
        
        fprintf('Running simulation: %.0fV, %d stages, max_time=%.3f s...\n', voltage, num_stages, max_time);
        ...
        gui_data.last_max_time = max_time;
        guidata(fig, gui_data);
        ...
        set(gui_data.status_text, 'String', sprintf('✓ Simulation completed\n%.0fV, %d stages, %.3f s\n→ %.6f m/s', ...
                                                    voltage, num_stages, max_time, result.velocity));
